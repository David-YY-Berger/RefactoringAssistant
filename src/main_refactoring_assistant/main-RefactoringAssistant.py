...
from src.add_suffix_to_csv import main_add_suffix_to_csv as mastc
...
def main_script():
    ...
    notes = ['Ensure that all file paths are ' + tc.UNDERLINE + "Absolute (not relative)" + tc.END + tc.NOTE_COLOR
             ...
             'Guides (PDF that explain how to use this app):\n\t\t' + p.path_for_guides + '\n',
    ...
    while True:
        ...
        print(" ")
        pf.print_note_list(notes, "A few things to remember:")
        ...
        elif cmd == enums.MainOptions.ADD_SUFFIX_TO_CSV.value:
            try:
                mastc.main_script()
            except Exception as e:
                gs.report_error(product_name=sfm.product_names.Add_Suffix_To_CSV.name, e=e)
