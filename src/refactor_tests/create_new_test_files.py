...
def create_ng_file(test_path, overwrite = False):
    try:
        ...
        if not overwrite:
            ...
            # overwriting...
            os.makedirs(os.path.dirname(dest_path), exist_ok=True)
            content_src = fu.read_file_content_as_str(test_path)
            fu.write_txt_to_file(dest_path, content_src)
            files_created += 1
            write_refactored_content(dest_path)
            pf.print_note("Overwrote content in : " + dest_path
                          +'\n\twith content in ' + test_path)
