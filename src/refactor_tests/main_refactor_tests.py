...
from src.gen_files import paths as p, gen_scripts as gs, create_data_files as cf, stats_for_metrics as sfm, file_utils as fu, test_class_funcs as tcf
...
def main_script():
    ...
    [num_ng_files_found, num_non_ng_files_found] = sttf.public_sort_tests_to_files(ng_path=p.ng_tests_dir_path, non_ng_path=p.non_ng_tests_dir_path,
    ...
    cp.public_create_playlist_from_files(p.output_dir, list_of_tests_path=p.non_ng_tests_dir_path)
    sfm.append_to_stats(version=ap.version, user_name=gs.get_cur_user_name(), date=gs.get_date(), time=gs.get_time(),
                        ...
                        non_ng_files_found=num_non_ng_files_found, ng_files_found=num_ng_files_found,
                        tests_analyzed=(num_non_ng_files_found+num_ng_files_found))
