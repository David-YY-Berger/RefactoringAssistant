...
from src.gen_files import file_utils as fu, gen_scripts as gs, constants as cons, enums as enums
from src.gen_files.ConsoleHelpers import print_functions as pf, input_helper as ih
from src.refactor_tests import create_playlist_from_files as cpff
...
def public_sort_tests_to_files(ng_path, non_ng_path, test_path_list):
    ...
    print(' ')
    return [len(ng_and_non_ng_tests[0]), len(ng_and_non_ng_tests[1])]
...
def priv_write_tests_to_output(ng_tests, non_ng_tests, ng_output_path, non_ng_output_path):
    ...
    pf.print_note("Wrote tests containing ng substrings to:\n\t" + ng_output_path)
...
def sort_ng_tests(test_path_lst):
    ...
    for f_path in test_path_lst:
        with open(f_path, 'rb', 0) as file:  # 'rb' - binary mode. '0' - no buffer
            ...
            # this is because of terrible design...
            [server_of_test, testng_file] = cpff.priv_get_server_for_test_path(f_path)
            if server_of_test == cons.not_find_testng or server_of_test == cons.empty_char:
                server_of_test = cons.empty_char
            ...
            if not is_ng_test:
                ...
                ng_tests.append(gs.space_nicely(test_name, max_length_test_name, str_to_add_ng_tests)
                                + '\n\truns on server: ' + server_of_test
                                + '\n\tfound in testng file: ' + testng_file)

    return [ng_tests, non_ng_tests]


def get_keyword_dict():
    if ap.default_keywords_choice == enums.DefaultKeywordsChoiceOptions.KEYWORDS1_ONLY.value:
        return ap.keywords1
    elif ap.default_keywords_choice == enums.DefaultKeywordsChoiceOptions.KEYWORDS2_ONLY.value:
        return ap.keywords2
    elif ap.default_keywords_choice == enums.DefaultKeywordsChoiceOptions.BOTH.value:
        return ap.keywords1 + ap.keywords2
    elif ap.default_keywords_choice == enums.DefaultKeywordsChoiceOptions.PROMPT_USER.value:
        print("\n")
        print("Keywords 1 = " + ap.keywords1_name)
        print("Keywords 2 = " + ap.keywords2_name)
        cmd = ih.get_input_enum_options("Which dictionary would you like to use?",
                                        list(enums.PromptUserKeywordOptions))
        keyword_choice = 'not yet chosen'
        if cmd == enums.PromptUserKeywordOptions.KEYWORDS1_ONLY.value:
            keyword_choice = ap.keywords1
        elif cmd == enums.PromptUserKeywordOptions.KEYWORDS2_ONLY.value:
            keyword_choice = ap.keywords2
        elif cmd == enums.PromptUserKeywordOptions.BOTH.value:
            keyword_choice = ap.keywords1 + ap.keywords2
        pf.print_note("Keywords chosen successfully. Processing...")
