...
def add_suffix_to_autotest_names(directory_path, suffix='AT', column='Autotest name'):
    ...
    for file_name in csv_files:
        ...
        df[column] = df[column].apply(lambda x: str(x) + str(suffix) if pd.notna(x) else str(suffix))
...
directory_path = r'C:\Users\davidbe\Downloads\New folder'
add_suffix_to_autotest_names(directory_path, 'Regression - Haim', 'Run')
