...
changes_in_non_ng_files_csv_path = os.path.join(p.discrep_tracker_output_dir, 'changes_in_non_ng_files.csv')

# output/report files:
discrep_output_dir = os.path.join(p.discrep_tracker_output_dir, 'discrepancies_by_section')
tests_passed_missing_ng_path_text_path = os.path.join(p.discrep_tracker_output_dir, 'tests_marked_as_passed_but_missing_ng_path.txt')
tests_to_mark_no_code_change_text_path = os.path.join(p.discrep_tracker_output_dir, 'tests_marked_as_passed_but_could_be_marked_as_no_code_change.txt')
tests_marked_passed_w_no_code_change_but_have_discrepancies_path = os.path.join(p.discrep_tracker_output_dir, 'tests_marked_passed_w_no_code_change_but_have_discrepancies.txt')


def main_script():

    # half of this file is in comment, bec I made a complex functionality that the staff rejected...
    # so you can ignore whatever code is in comment
    ...
    if cmd == enums.DiscrepancyTrackerOptions.FIND_AND_PROCCESS_DISCREPANCIES.value:
        ...
        # pf.print_step_separator('Step 3: (Optional) Overwrite discrepancies in non ng tests')
        ...
        [total_new_discreps, num_new_discrep_with_old_tests, num_new_discrep_with_new_tests] = (
            find_new_discrepancies(new_tests_new_discreps_path=new_tests_new_discreps_csv_path,
        ...
        print_discreps_per_section(input_csv=changes_in_non_ng_files_csv_path,
                                   discrep_description='between content of old and new non ng files')
        ...
        # print("")
        # pf.print_note('Wrote new discrepancies, in existing tests to: ' + old_tests_new_discreps_csv_path)
        # pf.print_note('Wrote new discrepancies, in new tests to: ' + new_tests_new_discreps_csv_path)
...
def find_and_write_discrepancies(test_obj_list, path_to_write):
    ...
    for test in (test_obj_list):
        ...
        if discrepancies:
            ...
            res = test.test_name + ":\n"
            res += get_discrepancies_str_by_function(discrepancies, src1_name=os.path.splitext(os.path.basename(test.test_path))[0],
    ...
    for test in test_objs_with_discrepancies:
        test.non_ng_content = fu.read_file_content_as_str(test.test_path)
...
def find_new_discrepancies(new_tests_new_discreps_path, old_tests_new_discreps_path):
    path_old_discrep_csv = ih.get_input_ensure_valid('Enter path to old discrepancies CSV', fu.is_csv_file,
                                                      'Please enter a valid csv path!')
    path_cur_discrep_csv = ih.get_input_ensure_valid('Enter path to current discrepancies CSV', fu.is_csv_file,
                                                      'Please enter a valid csv path!')
...
def print_discreps_per_section(input_csv, discrep_description):
    pf.print_note('Printed discrepancies (' +discrep_description + ') by section in directory: ' + discrep_output_dir)
...
def get_output_path_per_section(section):
    return os.path.join(discrep_output_dir, 'new_discrepancies_' + section + '_' + gs.get_date() + '.txt')
...
#             print()
#             num_tests_overwrote = len(test_obj_passed_no_code_change_with_discrepancies)
#             pf.print_note('Overwrote ' + str(num_tests_overwrote) + ' files. Remember to double check SVN merges.')
#             return num_tests_overwrote
#     return 0
