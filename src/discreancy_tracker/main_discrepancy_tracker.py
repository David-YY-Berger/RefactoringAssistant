...
# output/report files:
file_name_discrep_report = 'discrepancy_report'
discrep_by_section_output_dir = os.path.join(p.discrep_tracker_output_dir, 'discrepancy_report_by_section')
...
def main_script():
    ...
    # todo this function is way too long.. should be broken up to different functions.. but no time
    ...
    if cmd == enums.DiscrepancyTrackerOptions.FIND_AND_PROCCESS_DISCREPANCIES.value:
        ...
        [num_tests_with_no_path_ng, passed_tests_with_path_and_ng_paths] = get_passed_tests_w_ng_path_from_excel()
        ...
        pf.print_step_separator("Step 1: Finding tests marked as passed with no ng path")
        [num_tests_with_no_path_ng, passed_tests_with_path_and_ng_paths] = get_passed_tests_w_ng_path_from_excel()
        print(" ")
        pf.print_step_separator("Step 2: Discover new discrepancies")
        ...
        [total_new_discreps, num_new_discrep_with_old_tests, num_new_discrep_with_new_tests] = (
            find_all_new_discrepancies_and_print_to_dir(input_csv_dir_path))
        ...
        sfm.append_to_stats(version=ap.version, user_name=gs.get_cur_user_name(), date=gs.get_date(),
                            ...
                            tests_analyzed=total_new_discreps, passed_tests_with_no_ng_path=num_tests_with_no_path_ng)
...
def get_passed_tests_w_ng_path_from_excel():
    ...
    [num_tests_with_no_path_ng, passed_tests_with_path_and_ng_paths] = remove_tests_with_no_ng_path_and_print_by_section(
    ...
    pf.print_warning(
        "Found " + str(num_tests_with_no_path_ng) + " tests marked as 'passed' or 'passed without code change', "
                                                    ...
                                                    "Wrote these tests to: " + discrep_by_section_output_dir)
    return [num_tests_with_no_path_ng, passed_tests_with_path_and_ng_paths]
...
def remove_tests_with_no_ng_path_and_print_by_section(test_obj_list):
    ...
    sections = []
    for test in tests_with_no_path_ng:
        if test.test_section not in sections:
            sections.append(test.test_section)

        str_of_expected_path = tcf.get_nice_string_test_expected_path(test)
        fu.print_by_section_txt_file(content=str_of_expected_path, output_dir=discrep_by_section_output_dir,
                                     section=test.test_section, file_name=file_name_discrep_report,
                                     init_content=f"Discrepancy Report for {test.test_section}:\n")

    for section in sections:
        fu.print_by_section_txt_file(content='\n' + cons.line + '\n', output_dir=discrep_by_section_output_dir,
                                     section=section, file_name=file_name_discrep_report)
...
def print_discreps_per_section_if_exist(input_csv):
    ...
    for test in test_objs:
        ...
        fu.print_by_section_txt_file(content=discrep_str, output_dir=discrep_by_section_output_dir,
                                     section=test.test_section, file_name= file_name_discrep_report,
                                     init_content=f"""New Discrepancies found in {test.test_section}:
...
def find_all_new_discrepancies_and_print_to_dir(input_csv_dir_path):
