...
def main_script():
    ...
    if cmd == enums.DiscrepancyTrackerOptions.FIND_AND_PROCCESS_DISCREPANCIES.value:
        ...
        input_csv_dir_path = ih.get_input_ensure_valid(
            "Enter path to a directory containing all discrepancy CSVs. Make "
            "sure that directory does not contain any other files or directories!",
...
def find_all_new_discrepancies(input_csv_dir_path):
    ...
    pf.print_note('Printed discrepancies (between content of old and new non ng files) by section in directory: ' + discrep_by_section_output_dir)
    return [total_new_discreps, num_new_discrep_with_old_tests, num_new_discrep_with_new_tests]
