...
def get_paths_from_test_names(lst_test_names):
    ...
    for test_name in lst_test_names:
        ...
        test_path = get_file_path_from_name(test_name)
...
def reassign_paths_if_corrupted(test_obj):
    if not os.path.isfile(test_obj.test_path):
        res = get_file_path_from_name(test_obj.test_name)
        if res == cons.empty_char:
            pf.print_warning("Couldn't find (reg) path for: " + test_obj.test_name)
        else:
            test_obj.test_path = res
            test_obj.test_path_ng = get_ng_path_from_orig(test_obj.test_path)

    return test_obj


def get_file_path_from_name(file_name):
    return fu.get_val_from_key_csv(csv_path=p.init_dict_file_name_to_path, key_header=p.file_name_header,
                                        val_header=p.test_path_header, key=file_name)
...
def get_test_obj_list_from_test_name_list(test_name_list):
    res = []
    # to add the suffix to the test names
    test_name_list = [gs.add_suffix(test) for test in test_name_list]
...
def get_nice_string_list_test_name_section(test_obj_list, print_section = True):
...
def get_nice_string_test_expected_path(test):
    temp = test.test_name
    temp += '\n\tCurrent path (non ' + ap.ng_suffix + '): \n\t\t' + test.test_path
    temp += '\n\tExpected path: \n\t\t' + get_ng_path_from_orig(test.test_path) + '\n\n'
    return temp
...
def extract_function_headers_only(content_of_all_functions):
    if content_of_all_functions.__contains__(cons.no_discrepancies_found):
        return cons.could_not_locate_function
