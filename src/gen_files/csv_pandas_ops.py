...
def write_new_discrepancies_to_csvs(old_csv_path, cur_csv_path, old_test_new_discreps_path, new_tests_new_discreps_path):
    ...
    # Get column names except 'discrepancy'
    merge_columns = [col for col in cur_df.columns if col != Test.DISCREPANCY_FIELD]

    # Find discrepancies in old tests that are different from old_df
    merged_df = pd.merge(old_tests_df, old_df, on=merge_columns, suffixes=('_cur', '_old'))
    old_tests_new_discrep_df = merged_df[merged_df[Test.DISCREPANCY_FIELD +'_cur'] != merged_df[Test.DISCREPANCY_FIELD +'_old']]
