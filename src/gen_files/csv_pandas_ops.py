...
def write_paths_to_test_objs(test_obj_csv_path, test_name_path_testng_csv_path, output_path =''):
    # Read the CSV files

    test_to_paths_df = pd.read_csv(test_name_path_testng_csv_path)
    duplic_in_test_to_path = find_non_unique_values(test_name_path_testng_csv_path, p.test_name_header)
    if len(duplic_in_test_to_path) > 0:
        pf.print_warning("Found duplicates test names in project (" + test_name_path_testng_csv_path + '):\n' +
                         gs.open_list_as_string(duplic_in_test_to_path, '\n')
                         + '\n ignoring the duplicates..')
        test_to_paths_df = test_to_paths_df.drop_duplicates(subset='test_name', keep=False)
