...
def init_maps():
    ...
    if not os.path.exists(p.intelliJ_base_txt_file_path):
        with open(p.intelliJ_base_txt_file_path, 'w') as file:
            file.write(r'C:\urm\workspace-1.0.0.2-URM\alma_itest_ux\src\test')

    with open(p.intelliJ_base_txt_file_path, 'r') as file:
        p.intelliJ_base_path = file.read().strip()
    if (not os.path.exists(p.intelliJ_base_path)) or (not os.path.isdir(p.intelliJ_base_path)):
        ih.reassign_project_path()
    p.testng_dir_orig = os.path.join(p.intelliJ_base_path, "resources")
    p.all_tests_dir = os.path.join(p.intelliJ_base_path, "java", "tests")
