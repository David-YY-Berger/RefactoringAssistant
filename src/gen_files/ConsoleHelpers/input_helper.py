...
def get_test_objs_from_input(get_without_test_paths = False, get_source_type=False):
    """
    ...
    if cmd == src.gen_files.enums.OptionsInputTests.BY_DIRECTORY_PATH.value:
        ...
        test_obj_list = tcf.get_test_obj_list_from_path_list(path_list_without_basenames)
        src_type = src_dir
    else:
        if cmd == src.gen_files.enums.OptionsInputTests.BY_A_LIST_OF_TEST_NAMES.value:
            test_obj_list = tcf.get_test_obj_list_from_test_name_list(get_input_list("Enter a list of tests (only one test per line)"))
            ...
            test_paths =tcf.get_paths_from_test_names(test_names)
            test_obj_list = tcf.get_test_obj_list_from_path_list(test_paths)
        ...
        if not get_without_test_paths:
            test_names_list = [test.test_name for test in test_obj_list]
            paths = tcf.get_paths_from_test_names(test_names_list)
            # missing section.. need to add later
            # @todo
            test_obj_list = tcf.get_test_obj_list_from_path_list(paths)
...
def get_test_objs_from_excel():
    ...
    for chosen_sheet_name in chosen_sheets:
        ...
        for val in vals:
            test_obj_list.append(Test(test_name=val, test_section='Sheet: ' + chosen_sheet_name))
    return test_obj_list
...
def get_test_objs_from_csv():
    ...
    for chosen_section in chosen_sections:
        ...
        vals = [os.path.basename(item).split('.')[0] for item in vals]  # remove the '.test1'
        for val in vals:
            test_obj_list.append(Test(test_name=val, test_section='Section: ' + chosen_section))
    return test_obj_list
