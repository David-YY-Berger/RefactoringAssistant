...
def get_input_enum_options(prompt, enum_option_list, warning = None):
    print(prompt)
    if warning:
        pf.print_warning(warning)
    print(" (Enter just the number)")
    possible_options = []
    for op in enum_option_list:
...
def get_one_item_from_list(init_prompt, enter_list_prompt, option_list):
    pf.print_note('\n' + init_prompt)
    print(enter_list_prompt +
          "\nChoose only 1 item in the list")
    ...
    while keep_going:
        keep_going = False
        user_input = input()
        if not gs.is_valid_integer(user_input):
            keep_going = True
            pf.print_warning("Please enter a valid integer")
        else:
            value = int(user_input)
            if not gs.is_integer_in_range(value, 1, len(option_list)):
                keep_going = True
                pf.print_warning("Please enter an integer in range")
            else:
                return option_list[value-1]
...
def get_test_objs_from_excel():
    excel_file_path = get_input_ensure_valid("Enter the path to the Excel file containing the tests: ", fu.is_excel_file, "Please enter a path to a valid excel file")
...
def get_test_objs_from_csv():
    csv_file_path = get_input_ensure_valid("Enter the path to the csv file: ", fu.is_testrail_csv,
