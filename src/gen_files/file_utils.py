...
def print_by_section_txt_file(content, output_dir, section, file_name, init_content = ""):
    existing_files = get_files_from_dir_as_list(output_dir, is_txt_file, " a text file")
    path_to_section_discrepancies = get_output_path_per_section(section, output_dir, file_name)
    if path_to_section_discrepancies not in existing_files:
        existing_files.append(path_to_section_discrepancies)
        content_to_write = init_content + '\n' + content
        write_txt_to_file(path=get_output_path_per_section(section, output_dir, file_name),
                             content=content_to_write)
        # pf.print_note(section)
    else:
        append_txt_to_file(content=content, path=path_to_section_discrepancies)


def get_output_path_per_section(section, output_dir, file_name):
    return os.path.join(output_dir, file_name + '_' + section + '_' + gs.get_date() + '.txt')
