...
def public_create_playlist_from_files(dest_dir, non_ng_tests_path):
    print("Creating playlists for non ng tests... please wait")
...
def priv_create_all_playlists(all_lists, dest_dir):
    priv_create_playlist_from_test_list(all_lists[src.gen_files.enums.Servers.QAC01.value], dest_dir, src.gen_files.enums.Servers.QAC01.name)
    priv_create_playlist_from_test_list(all_lists[src.gen_files.enums.Servers.SQA_NA01.value], dest_dir, src.gen_files.enums.Servers.SQA_NA01.name)
    priv_create_playlist_from_test_list(all_lists[src.gen_files.enums.Servers.SQA_EU01.value], dest_dir, src.gen_files.enums.Servers.SQA_EU01.name)
    priv_create_playlist_from_test_list(all_lists[src.gen_files.enums.Servers.COULD_NOT_FIND_SERVER.value], dest_dir, src.gen_files.enums.Servers.COULD_NOT_FIND_SERVER.name)
...
def priv_get_lists_by_server(test_path_lst):
    res = [[] for _ in range(len(list(src.gen_files.enums.Servers)))]
    ...
    for test_path in test_path_lst:
        ...
        if server == cons.empty_char:
            res[src.gen_files.enums.Servers.COULD_NOT_FIND_SERVER.value].append(test_path)
        elif server == src.gen_files.enums.Servers.QAC01.name:
            res[src.gen_files.enums.Servers.QAC01.value].append(test_path)
        elif server == src.gen_files.enums.Servers.SQA_NA01.name:
            res[src.gen_files.enums.Servers.SQA_NA01.value].append(test_path)
        elif server == src.gen_files.enums.Servers.SQA_EU01.name:
            res[src.gen_files.enums.Servers.SQA_EU01.value].append(test_path)
        elif server == src.gen_files.enums.Servers.IGNORE.name:
            res[src.gen_files.enums.Servers.IGNORE.value].append(test_path)
        elif server == cons.not_find_testng:
            tests_with_no_testng.add(os.path.splitext(os.path.basename(test_path))[0])


    if len(tests_with_no_testng) > 0:
        print("\nNo testng file found for the following tests:\n" + gs.open_list_as_string(list(tests_with_no_testng), "\n" ),
              "Please add a testng file and server in " + p.testng_server_csv_path + "\n"
            "Choose from: " + gs.open_list_as_string(list(s.name for s in src.gen_files.enums.Servers)) + "\n")
    if len(testng_with_no_server) > 0:
        print("\nNo server found for the following testng files:\n" + gs.open_list_as_string(list(testng_with_no_server), "\n") +
              "Please add a server in " + p.testng_server_csv_path + "\n"
            "Choose from: " + gs.open_list_as_string(list(s.name for s in src.gen_files.enums.Servers)) + "\n")
